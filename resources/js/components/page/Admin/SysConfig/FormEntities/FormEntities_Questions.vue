<template>
    <div class="sysconfig">
        <div class="w-full mx-auto mt-4 ml-20 rounded">
            <p class="text-2xl">Form Entities (Groupings, Questions and Choices)</p>
            <div class="separator"></div>
            <!-- Tabs -->
            <ul id="tabs" class="inline-flex w-full mt-4 ">
                <li class="px-4 py-2 -mb-px font-semibold text-gray-800 border-b-2 border-blue-400 rounded-t opacity-50">
                    <a id="default-tab" href="#first">Form Questions Group Setup</a>
                </li>
                <li class="px-4 py-2 font-semibold text-gray-800 rounded-t opacity-50">
                    <a href="#second">Form Questions Setup</a>
                </li>
                <li class="px-4 py-2 font-semibold text-gray-800 rounded-t opacity-50">
                    <a href="#third">Form Questions List</a>
                </li>
            </ul>
            <!-- Tab Contents -->
            <div id="tab-contents" class="bg-white w-fullx">
                <div id="first" class="p-4">
                    <FE_Questions_Group />
                </div>
                <div id="second" class="p-4">
                    <FE_Questions_Setup />
                </div>
                <div id="third" class="hidden p-4">
                    <FE_Questions_List />
                </div>
            </div>
        </div>
    </div>
</template>
<script>
import FE_Questions_Group from "./FE_Questions_Group";
import FE_Questions_Setup from "./FE_Questions_Setup";
import FE_Questions_List from "./FE_Questions_List";
import FormEntitiesFunctions from "../../../../../plugins/Modular/FormEntities/FormEntitiesFunctions";
export default {
    mixins: [
        FormEntitiesFunctions
    ],
    components: {
        FE_Questions_Group,
        FE_Questions_Setup,
        FE_Questions_List
    },
    created() {
        this.$root.setUserInfo();
    }
}
</script>

<style scoped>
@import '../SysConfig.css';
</style>
